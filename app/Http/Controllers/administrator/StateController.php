...
class StateController extends Controller
{
    ...
    public function create()
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            $countryObj = Country::all();
            $tablename = 'state';
            return view('administrator/state.create',compact('tablename'))
            ->with('countryObj', $countryObj);
        }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
    ...
    public function show($id)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $tablename = 'state';
                return view('administrator/state.show', compact('state','seocontent','newUpdatedFields','tablename'));
            }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
    ...
    public function edit($id)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $tablename = 'state';
                return view('administrator/state.edit', compact('state','seocontent','newUpdatedFields','tablename'))
                ->with('countryObj', $countryObj);
            }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
    ...
    public function allStateSearch(Request $request){
