...
class CareerController extends Controller
{
    ...
    public function index()
    {
        if (Auth::check())
        {
            ...
            return view('administrator/career.index', compact('career'));
            }else{
    ...
    public function create()
    {
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
               ...
               return view('administrator/career.create')
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                return redirect('administrator/career');
            }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
               //$career = Career::findOrFail($id);
                ...
                return view('administrator/career.show')
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
               $career = Career::findOrFail($id);
                ...
                $addressDataObj = DB::table('careers')
                                ->leftJoin('city','careers.city_id','=','city.id')
                                ->leftJoin('state','city.state_id','=','state.id')
                                ->select('careers.id','city.id as cityId','city.name as cityName','state.id as stateId','state.name as stateName','dateOfBirth')
                                ->where('careers.id', '=', $id)
                                ->take(1)
                ...
                return view('administrator/career.edit', compact('career'))
                ->with('cityNameObj', $cityNameObj)
                ->with('stateNameObj', $stateNameObj)
