...
class UserPrivilegeController extends Controller
{
    ...
    public function index()
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            $userprivilege = userPrivilege::orderBy('userprivileges.id', 'DESC')
                            ...
                            ->paginate(20, array('userprivileges.id', 'users.firstname', 'users.lastname', 'alltableinformations.name as tableName','userprivileges.slug','users.id as usersId','eID.id as eUserId','eID.firstname as employeeFirstname', 'eID.middlename as employeeMiddlename', 'eID.lastname as employeeLastname','userprivileges.updated_at'))
                            ;
            
            return view('administrator/userprivilege.index', compact('userprivilege'));
        }else{
    ...
    public function create()
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            return view('administrator/userprivilege.create')
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            if( !empty($validateRows) ){
                foreach ($validateRows as $key => $value) {
                    ...
                    return redirect()->action('administrator\UserPrivilegeController@edit', [$value->id]);
                }
            }else{
                //DATA SAVE IN DB
                    ...
                    foreach($allUserGroupTableInfoID as $key ) {
                        ...
                        if($checkTableIdInfoObj == '0') {
                            ...
                            if(env('APP_ENV') == 'local'){
                               $dirUrl = url().'/administrator/usergroup-table-info/'.$getGroupName[0]->slug;
                            }else{
                                $dirUrl = url().'/administrator/usergroup-table-info/'.$getGroupName[0]->slug;
                            }
                            ...
                            return redirect('administrator/userprivilege');
                        }
    ...
    public function show($id)
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            return view('administrator/userprivilege.show', compact('userprivilege'));
        }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
           ...
           // $allTableInfoObj = AllTableInformation::all();
            $allTableInfoObj = DB::table('alltableinformations')
                    ...
                    ;
            $allUsersObj = DB::table('users')->where('userrole_id', '=', '4')->get();
            return view('administrator/userprivilege.edit', compact('userprivilege'))
            ->with('allTableInfoObj', $allTableInfoObj)
            ->with('allUsersObj', $allUsersObj)
            ;
        }else{
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
           ...
           // $allTableInfoObj = AllTableInformation::all();
            $allTableInfoObj = DB::table('alltableinformations')
                    ...
                    ;
            $allUsersObj = DB::table('users')->where('userrole_id', '=', '4')->get();
            return view('administrator/userprivilege.userPrivilegeInedx', compact('userprivilege'))
                ->with('allTableInfoObj', $allTableInfoObj)
                ->with('allUsersObj', $allUsersObj)
