...
class CollegeProfileController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            return view('administrator/collegeprofile.index', compact('collegeprofile'))
    ...
    public function create()
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            return view('administrator/collegeprofile.create')
    ...
    public function store(Request $request)
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            return redirect('administrator/collegeprofile');
        }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
    ...
    public function show($id)
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
           ...
           // $collegeprofile = CollegeProfile::findOrFail($id);
            return view('administrator/collegeprofile.show', compact('collegeprofile','seocontent','socialMediaLinksDataObj','collegeRatingObj'))
    ...
    public function edit($id)
    {
        ...
        // print_r($collegeDataObj);die;
            ...
            return view('administrator/collegeprofile.edit', compact('collegeprofile','seocontent','socialMediaLinksDataObj'))
            ->with('usersObj', $usersObj)
            ->with('collegeTypeObj', $collegeTypeObj)
            ->with('universityObj', $universityObj)
    ...
    public function update($id, Request $request)
    {
        ...
        if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
            ...
            /*//SEND EMAILS ON REVIEW IS ON 1
            if((Input::get('review') == '1') && ($this->fetchDataServiceController->isValidEmail($profileOwnerEmailAddress) == 1)){
                try {
                    //Swift Mailer Data Fetching     
                    \Mail::send('administrator/collegeprofile/emails.review-success', array('email' => $profileOwnerEmailAddress, ), function($message) use ($profileOwnerEmailAddress)
                    {
                        $message->to($profileOwnerEmailAddress, 'AdmissionX')->subject('Your college profile has been successfully reviewed.');
                    });       
                } catch ( \Swift_TransportException $e) {                
     ...
     * @return Response
     */
    public function searchCollegeProfile(Request $request)
    {
...
// AND address.addresstype_id = '1'
    ...
    public function allCollegeProfile(Request $request){
    ...
    public function collegeContactCard(Request $request)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $countryObj = Country::all();
                return view('administrator/collegeprofile.college-contact-card', compact('collegeprofile'))
                        ->with('universityObj', $universityObj)
                        ->with('collegeProfileObj',$collegeProfileObj)
                        ->with('countryObj', $countryObj);
    ...
    public function sendWelcomeEmail(Request $request , $id)
    {
        ...
        try {
            if(!empty($collegeEmailId))
            {
                ...
                get_headers('https://www.admissionx.com/');
    ...
    public function isShowOnHome(Request $request)
    {
        ...
        $id = Input::get('id');
        DB::statement(DB::raw("UPDATE collegeprofile SET isShowOnHome=".Input::get('currentStatus')." WHERE id=".Input::get('id').""));
    ...
    public function isShowOnTop(Request $request)
    {
        $status = Input::get('currentStatus');
        $id = Input::get('id');
        DB::statement(DB::raw("UPDATE collegeprofile SET isShowOnTop=".Input::get('currentStatus')." WHERE id=".Input::get('id').""));
        return response()->json(['code' => 200, 'response' => 'success']);
    }
