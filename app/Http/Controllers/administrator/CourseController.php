...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $degreeObj = Degree::orderBy('degree.id', 'DESC')
                                ->leftJoin('functionalarea', 'degree.functionalarea_id', '=', 'functionalarea.id')
                                ->select('degree.id', 'degree.name', 'functionalarea.name as functionalareaName')
                                ->get();
    ...
    public function create()
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $tablename = 'course';
                return view('administrator/course.create', compact('tablename'))
                    ->with('degreeObj', $degreeObj)
    ...
    public function show($id)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                return view('administrator/course.show', compact('course','seocontent','newUpdatedFields','tablename'));
            }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
    ...
    public function edit($id)
    {
        ...
        if (Auth::check())
        {
            ...
            if( $roleGrant->userrole_id == '1' && $roleGrant->userstatus_id == '1' ){
                ...
                $newUpdatedFields = $this->fetchDataServiceController->fetchNewUpdatedFields('Course','course',$id);
                $tablename = 'course';
                return view('administrator/course.edit', compact('course','seocontent','newUpdatedFields','tablename'))
                ->with('degreeObj', $degreeObj)
                ->with('functionalAreaObj',$functionalAreaObj);
            }else{
                Auth::logout(); // logout user
    ...
    public function allCourseSearch(Request $request){
    ...
    public function deleteSearchCourse(Request $request, $id)
    {   
        Course::destroy($id);
