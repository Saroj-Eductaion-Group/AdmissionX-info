        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                ...
                $collegeProfileObj = DB::table('collegeprofile')
                                    ...
                                    ->get()
                                    ;
                return view('employee/event.index', compact('event'))
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                $collegeProfileObj = DB::table('users')
                        ...
                        ->get()
                        ; 
                return view('employee/event.create')
                ->with('collegeProfileObj',$collegeProfileObj);
            }else{
                Session::flash('access_restricted_msg', 'Access Restricted!');
                return Redirect::action('employee\AdminEmployeeController@index');
    ...
    public function store(Request $request)
    {
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
           /* Event::create($request->all());
            ...
            return redirect('employee/event');
        }else{
                Auth::logout(); // logout user
                return Redirect::to('login'); //redirect back to login
            }
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                ...
                $collegeObj = Event::where('event.id', $event->id)
                                            ...
                                            ->where('users.userrole_id','=', '2')
                                            ->get()->first();
                return view('employee/event.edit', compact('event'))
                ->with('collegeProfileObj',$collegeProfileObj)
...
/**
     ...
     * @return Response
     */
    public function collegeEventEmployeeSearch(Request $request)
    {
        $search0 = 'event.id';
    ...
    public function allEventEmployeeSearch(Request $request){
    ...
    public function deleteEmployeeSearchEvent(Request $request, $id)
    {   
        Event::destroy($id);
