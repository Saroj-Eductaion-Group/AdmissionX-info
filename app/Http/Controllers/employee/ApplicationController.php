...
class ApplicationController extends Controller
{
    ...
    public function index()
    {
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                ...
                return view('employee/application.index', compact('application'))
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                ...
                $collegeProfileObj = DB::table('users')
                            ...
                            ->get()
                            ;
                return view('employee/application.create')
                ->with('collegeProfileObj',$collegeProfileObj)
                ->with('applicationStatusObj',$applicationStatusObj)
                ->with('userObj',$userObj);
            }else{
            ...
            if( $validateUrlUsers >= '1' ){
                $application = Application::orderBy('id', 'DESC')
                 ...
                 $validateUserRoleAction = DB::table('users')
                                    ...
                                    ->where('users.id', '=', $userId)
                                    ->where('alltableinformations.name', '=', 'Application')
                                    ->where('userprivileges.metrics1', '=', '1')
                                    ->select('userprivileges.metrics1')
                                    ->orderBy('userprivileges.id', 'DESC')
                                    ->take(1)
                                    ->get()
                                    ;
                if(!empty($validateUserRoleAction)){
                    $storeProvisionalLetterAction = $validateUserRoleAction[0]->metrics1;
                }else{
                    $storeProvisionalLetterAction = "";
                }

                return view('employee/application.show', compact('application'))
                        ->with('storeProvisionalLetterAction', $storeProvisionalLetterAction);
            }else{
                Session::flash('access_restricted_msg', 'Access Restricted!');
                return Redirect::action('employee\AdminEmployeeController@index');
            }
    ...
    public function edit($id)
    {
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
            ...
            if( $validateUrlUsers >= '1' ){
                ...
                $userObj = User::all();
                $applicationStatusObj = ApplicationStatus::all();
                return view('employee/application.edit', compact('application'))
                        ->with('collegeProfileObj',$collegeProfileObj)
                        ->with('applicationStatusObj',$applicationStatusObj)
                        ->with('userObj', $userObj)
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' ){
           ...
           // $application->collegeprofile_id = $collegeProfileDataObj[0]->collegeProfileId;
            ...
            /********** Update Application Remarks ********************/
                ...
                $applicationStatusMessageObj->others = 'Employee Remarks';
            ...
            {  
                try {
                    ...
                    {   
                        ...
                        // $smsMessageData = 'Hi '.$studentName.', '.$collegeName.' has accepted your application. Congratulations! You will shortly receive the provisional admission letter through email. '.Config::get('systemsetting.SMS_GROUP_NAME_3');  
                        // $resultSet = $this->fetchDataServiceController->sendTextSmsOnMobile($userMobileNo, $smsMessageData, Config::get('systemsetting.TEMPLATE_APPLICATION_STATUS_CHANGED'));
            ...
            {  
                try {
                    ...
                    {   
                        ...
                        $smsMessageData = 'Hi '.$studentNameSms.', '.(str_limit($collegeNameStr, $limit = 30, $end = '')).' '.env('APPLICATIONREJECT').' '.Config::get('systemsetting.SMS_GROUP_NAME_3');
                        $userMobileNo = $studentPhoneNo;  
                    ...
                    try {
                        ...
                        if(!empty($adminEmailId) && ($this->fetchDataServiceController->isValidEmail($adminEmailId) == 1))
                        {
                           /**Swift Mailer Data TO admin_id***/        
                            \Mail::send('employee/application.email.cancelAdmission', array('email' => $adminEmailId, 'messageData' => $messageText,'applicationstatusName' => $applicationstatusName  ), function($message) use ($adminEmailId)
                            {
    ...
    public function applicationEmployeeSearch(Request $request)
    {
    ...
    public function allApplicationEmployeeSearch(Request $request){
    ...
    public function sendProvisionalLetter(Request $request , $id)
    {
        ...
        if( $roleGrant->userrole_id == '4' && $roleGrant->userstatus_id == '1' )
        {
           $getStudentEmailAddress = DB::table('application')
                            ...
                            ->leftJoin('users as u2','u2.id','=','collegeprofile.users_id')
                            ->where('application.id','=', $id)
                            ->select('u1.email as studentEmail','u2.email as collegeEmail','application.applicationstatus_id','applicationstatus.name','application.collegemaster_id','u1.firstname as StudentFirstName','u1.middlename as studentMiddleName','u1.lastname as studentLastName','u2.firstname as collegeName','application.applicationID','u1.phone as studentPhone','u2.phone as collegePhone')
                            ->get()
                            ;
            ...
            $pathToFile = env('APP_URL').'/assets/images/provisionalletter.pdf/';
            ...
            try {
                ...
                {   
                    /**Swift Mailer TO Student***/    
                    \Mail::send('employee/application.email.provisionalletter.studentEmailTemplate', array('email' => $studentEmailId,'studentName'=>$studentName,'applicationID'=>$applicationID,'validDate'=>$validDate,'filePath'=>$pathToFile ), function($message) use ($studentEmailId)
                    {
            ...
            try {
                ...
                {
                        /**Swift Mailer TO COLLEGE***/        
                        \Mail::send('employee/application.email.provisionalletter.collegeEmailTemplate', array('email' => $collegeEmailId,'collegeName'=>$collegeName,'studentName'=>$studentName,'applicationID'=>$applicationID,'filePath'=>$pathToFile ), function($message) use ($collegeEmailId)
                        {
            ...
            try {
                if(!empty($studentMobileNo))
                {
                   ...
                   // $smsMessageData = 'Hi '.$studentNameSms.', you have been emailed the provisional admission letter for application id '.$applicationId.' Please read the enclosed T&C's carefully. This letter is valid until 2016-08-31'; 
