...
<div class="form-group">
    <div class="col-md-12">
        ...
        <select class="form-control chosen-select" name="status" data-parsley-error-message=" Please select status" data-parsley-trigger="change" required="">
            ...
            <option value="1" @if(isset($examinationmode) && $examinationmode->status == 1) selected="" @endif>Active</option>
            <option value="0" @if(isset($examinationmode) && $examinationmode->status == 0) selected="" @endif>Inactive</option>
        </select>
    </div>
</div>
