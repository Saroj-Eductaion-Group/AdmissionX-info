...
Lists of Comment has been submitted on your behalf
@endsection
					...
					<div class="profile-bio">
						<div class="row">
							<div class="col-md-9">
								...
								@endif
								<span><strong>Comment has been submitted on your behalf</strong></span>
							</div>				
						</div>
					</div>
					<hr class="hr-gap">
					<div class="detail-page-signup margin-bottom40 table-responsive">
						<div class="headline"><h2>Here is the list of comment submitted by you</h2></div>
						<!-- Updated Course List -->
						@if( sizeof($listOfSubmitComments) > 0 )
							@foreach( $listOfSubmitComments as $item )
								<div class="row blog blog-medium margin-bottom-40 clientContactDetails">
									<div class="col-md-12">
										...
										<h4>
											<span class="text-left">
												Answer : @if($item->status == 1)<a  href="{{ URL::to('ask', $item->slug) }}" target="_blank">{!! $item->answer !!}</a> @else {!! $item->answer !!} @endif
											</span>
										</h4>
										<h5>
											<span class="text-left">
												Your Comment : @if($item->status == 1)<a  href="{{ URL::to('ask', $item->slug) }}" target="_blank">{!! $item->replyanswer !!}</a> @else {!! $item->replyanswer !!} @endif
											</span>
										</h5>
										<ul class="list-unstyled list-inline blog-info">
											<li><i class="fa fa-calendar"></i> Comment Date : {{ date('d F Y h:s a', strtotime($item->answerDate)) }}</li>
										</ul>
									</div>
								</div>
							@endforeach
							<div class="row indexPagination">
                                ...
                                <div class="custom-pagination">{!! $listOfSubmitComments->render() !!}</div>
	                            </div>
	                        </div>
						@else
