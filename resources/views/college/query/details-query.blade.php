				...
				<div class="profile-body">
					...
					<div class="row">
						<div class="col-sm-12 sm-margin-bottom-30">
							<div class="panel panel-profile">
								...
								<div class="panel-body">
									<ul class="list-unstyled social-contacts-v2">
										@if( $getQueryCollegeDataObj )
											@foreach( $getQueryCollegeDataObj as $item )
												<li><i class="fa fa-user"></i>Student : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
											@endforeach
										@endif
										...
										@if( $getQueryDataForReplies )
											@foreach( $getQueryDataForReplies as $item )
												@if( $item->queryflowtype != 'student-to-college' )
													<li class="text-right"><i class="fa fa-graduation-cap "></i>{{ $collegeFullName }} : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
												@else
													<li class="text-left"><i class="fa fa-user"></i>Student : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
												@endif
											@endforeach
										@endif											
									</ul>
							...
							<div class="panel panel-profile">
								<div class="panel-body">
									<form action="/college/update-comment-query" method="POST">
