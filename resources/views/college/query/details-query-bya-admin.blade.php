...
<div class="wrapper">
	<div class="container content profile">
		<div class="row">
			<div class="col-md-12 text-right"><a href="{{ URL::to('college/check-queries', $slug) }}" class="btn btn-warning"><i class="fa fa-arrow-left"></i> Back</a></div>
			<div class="col-md-12">
				<div class="profile-body">
							...
							<div class="col-md-12">
								@if( $getCollegeProfileObj )
									...
									@foreach( $getCollegeProfileObj as $item )
										{{--*/ $collegeFullName = $item->firstname; /*--}}
						...
						<div class="row margin-top10">
							<div class="col-md-6">
								@if( $getQueryCollegeDataObj )
									@foreach( $getQueryCollegeDataObj as $item )
					...
					<div class="row">
						<div class="col-sm-12 sm-margin-bottom-30">
							<div class="panel panel-profile">
								...
								<div class="panel-body">
									<ul class="list-unstyled social-contacts-v2">
										@if( $getQueryCollegeDataObj )
											@foreach( $getQueryCollegeDataObj as $item )
												<li><i class="fa fa-graduation-cap"></i>Colllege : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
											@endforeach
										@endif
										...
										@if( $getQueryDataForReplies )
											@foreach( $getQueryDataForReplies as $item )
												@if( $item->queryflowtype != 'college-to-admin' )
													<li class="text-right"><i class="fa fa-users"></i>Admission X Admin : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
												@else
													<li class="text-left"><i class="fa fa-graduation-cap"></i>College : {{ $item->message }} | {{ date('M d, Y ', strtotime($item->created_at)) }}</li>
												@endif
											@endforeach
										@endif											
									</ul>
							...
							<div class="panel panel-profile">
								<div class="panel-body">
									<form action="/college/update-comment-query-to-bya" method="POST">
										<input type="hidden" name="chatkey" value="{{ $chatkey }}">
										<div class="row">
											<div class="col-md-12">
												<textarea name="message" class="form-control" rows="3" maxlength="250" placeholder="Reply to college" required=""></textarea>
