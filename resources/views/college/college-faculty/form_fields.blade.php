...
/*<!-- <div class="facultyDepartmentSection">
    ...
    <div class="clientContactDetails margin-bottom10">
	    ...
	    <div class="row padding-top5 padding-bottom5">
	    	<div class="col-md-12">
	    		<label>Course</label>
	            <select name="collegemaster_id[]" class="form-control chosen-select " data-parsley-error-message=" Please select course" data-parsley-trigger="change" required="">
                    <option value="" selected disabled>Select course </option>   
                    @foreach ($allCourseObj as $course)                    	
	                   	<option value="{{ $course->collegemasterId }}" @if( $item->collegemaster_id == $course->collegemasterId ) selected="" @endif>{{ $course->courseName }} (Degree : {{$course->degreeName}} | Stream : {{$course->functionalareaName}}) (Course Type : {{ $course->coursetypeName }}) (Degree Level : {{ $course->educationlevelName }})</option>
	                @endforeach       
                </select>
	        </div>
	    </div>
