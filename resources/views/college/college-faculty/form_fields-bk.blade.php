...
<div class="row">
	...
	<div class="col-md-3">
		...
		<input type="text" class="form-control" name="designation" placeholder="Enter designation here" data-parsley-error-message = "Please enter valid designation" data-parsley-pattern="^[a-zA-Z\s .]*$" data-parsley-trigger="change" value="{{ $getFacultyObj->designation }}">
	</div>
</div>
<hr class="hr-gap">
<div class="row">
	<div class="col-md-12">
		...
		<input type="text" class="form-control" name="languageKnown" placeholder="Enter language known here" data-parsley-error-message = "Please enter valid language known" data-parsley-pattern="^[a-zA-Z\s .,]*$" data-parsley-trigger="change" value="{{ $getFacultyObj->languageKnown }}">
	</div>
</div>
<hr class="hr-gap">
<div class="row padding-top5 padding-bottom5">
...
<div class="facultyDepartmentSection">
	@if(isset($facultyDepartmentObj))
	@foreach($facultyDepartmentObj as $key => $item)
    <div class="clientContactDetails margin-bottom10">
	    	...
	    	<div class="col-md-4">
	            ...
	            <select name="course_id[]" class="form-control chosen-select " data-parsley-error-message=" Please select course" data-parsley-trigger="change" required="">
                    ...
                    @foreach ($courseObj as $course)
                    	@if( $item->course_id == $course->courseId )
	                    	<option value="{{ $course->courseId }}" selected="">{{ $course->courseName }} (Degree : {{$course->degreeName}} | Stream : {{$course->functionalareaName}})</option>
                    	@else
                    		<option value="{{ $course->courseId }}">{{ $course->courseName }} (Degree : {{$course->degreeName}} | Stream : {{$course->functionalareaName}})</option>
                		@endif			                    			                    
	                @endforeach       
                </select>
	        </div>
	    	<div class="col-md-4">
	            ...
	            <select name="educationlevel_id[]" class="form-control chosen-select " data-parsley-error-message=" Please select degree level" data-parsley-trigger="change" required="">
                    ...
                    @foreach ($educationLevelObj as $education)
                    	@if( $item->educationlevel_id == $education->id)
	    	...
	    	<div class="col-md-4">
	            ...
	            <select name="coursetype_id[]" class="form-control chosen-select " data-parsley-error-message=" Please select course type" data-parsley-trigger="change" required="">
                    ...
                    @foreach ($courseTypeObj as $courseType)
                    	@if( $item->coursetype_id == $courseType->id )
...
<div class="row">
    <div class="col-md-3 pull-right margin-top20">
        <a href="javascript:void(0);" class="btn btn-block btn-sm btn-success" id="addNewDepartmentDetailRow"><i class="fa fa-plus"></i> Add Associate Department</a>
    </div>
</div>