        ...
        @if(Auth::check())
            @if(Auth::user()->userrole_id == 4)
                @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->create == '1'))
                    <h2>Popular Career Details <a href="{{ url('counseling/counseling-career-details/create') }}" class="btn btn-primary pull-right btn-sm">Add New Popular Career Details</a></h2>
                @endif
            @else
                <h2>Popular Career Details <a href="{{ url('counseling/counseling-career-details/create') }}" class="btn btn-primary pull-right btn-sm">Add New Popular Career Details</a></h2>
            @endif
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-content">
                <div class="row">
                    <div class="col-lg-10">
                        <h2>Search Popular Career Details</h2>        
                    </div>    
                </div>

                <form action="{{ URL::to('/counseling/counseling-career-details') }}" method="GET">
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-3 pull-right text-right margin-top20">
                            <a href="{{ URL::to('/counseling/counseling-career-details') }}" class="btn btn-md btn-primary">Clear</a>
...
<div class="row wrapper border-bottom page-heading margin-top20">
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <table class="table table-hover">
                    ...
                    <tbody class="tbody">
                        @foreach($counselingcareerdetails as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->title }}</td>
                            <td> 
                                ...
                                @if(Auth::check())
                                    @if(Auth::user()->userrole_id == 4)
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->show == '1'))
                                            <a href="{{ url('counseling/counseling-career-details/' . $item->id) }}">
                                                <button type="submit" class="btn btn-info btn-xs">Show</button>
                                            </a>
                                        @endif
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->edit == '1'))
                                            <a href="{{ url('counseling/counseling-career-details/' . $item->id . '/edit') }}">
                                        ...
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->delete == '1'))
                                            {!! Form::open([
                                                'method'=>'DELETE',
                                                'url' => ['counseling/counseling-career-details', $item->id],
        ...
        <div class="pagination"> {!! $counselingcareerdetails->render() !!} </div>
    </div>
</div>
@endsection
