        ...
        @if(Auth::check())
            @if(Auth::user()->userrole_id == 4)
                @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->create == '1'))
                    <h2>Counseling Career Relevant Details <a href="{{ url('counseling/counseling-career-relevant/create') }}" class="btn btn-primary pull-right btn-sm">Add New Counseling Career Relevant</a></h2>
                @endif
            @else
                <h2>Counseling Career Relevant Details <a href="{{ url('counseling/counseling-career-relevant/create') }}" class="btn btn-primary pull-right btn-sm">Add New Counseling Career Relevant</a></h2>
            @endif
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-content">
                <div class="row">
                    <div class="col-lg-10">
                        <h2>Search Counseling Relevant Details</h2>        
                    </div>    
                </div>

                <form action="{{ URL::to('/counseling/counseling-career-relevant') }}" method="GET">
                    <div class="row">
                        <div class="col-md-4">
                            <h4>Career Intrest<span class="pull-right"> <a href="javascript:void(0);" id="refresh1" class="hide"><i class="fa fa-remove"></i></a></span></h4>
                           <select class="form-control chosen-select careerInterest" name="careerInterest" data-parsley-trigger="change" data-parsley-error-message="Please select stream">
                                <option value="" disabled="" selected="">Select stream</option>
                                @foreach( $counselingCareerInterestObj as $functional )
                                    <option value="{{ $functional->id }}" @if(Request::get('careerInterest') == $functional->id) checked="" @endif>{{ $functional->functionalAreaName }} : {{ $functional->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-2 pull-right text-right margin-top20">
                            <a href="{{ URL::to('/counseling/counseling-career-relevant') }}" class="btn btn-md btn-primary">Clear</a>
...
<div class="row wrapper border-bottom page-heading margin-top20">
    <div class="col-lg-12">
                ...
                <table class="table table-hover">
                    ...
                    <tbody class="tbody">
                        ...
                        <tr>
                            ...
                            <td>{{ $item->title }}</td>
                            <td>{{ $item->stream }}</td>
                            <td>{{ $item->mandatorySubject }}</td>
                            <td>{{ $item->academicDifficulty }}</td>
                            <td><a href="{{ url('counseling/counseling-career-interests', $item->counseling_career_interestsID) }}">{{ $item->functionalAreaName }} : {{ $item->interestsTitle }}</a></td>
                            <td> 
                                @if( $item->image )
                                ...
                                @if(Auth::check())
                                    @if(Auth::user()->userrole_id == 4)
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->edit == '1'))
                                            <a href="{{ url('counseling/career/update-form-details/' . $item->id) }}">
                                        ...
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->show == '1'))
                                            / <a href="{{ url('counseling/counseling-career-relevant/' . $item->id) }}">
                                                <button type="submit" class="btn btn-info btn-xs">Show</button>
                                            </a>
                                        @endif
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->edit == '1'))
                                            / <a href="{{ url('counseling/counseling-career-relevant/' . $item->id . '/edit') }}">
                                        ...
                                        @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->delete == '1'))
                                            / {!! Form::open([
                                                'method'=>'DELETE',
                                                'url' => ['counseling/counseling-career-relevant', $item->id],
        ...
        <div class="pagination"> {!! $counselingcareerrelevant->render() !!} </div>
    </div>
</div>
@endsection
...
<script type="text/javascript">
    $('.careerInterest').on('change',function(){
        $('#refresh1').removeClass('hide');
    });
    $('#refresh1').on('click',function(e){
        $('.careerInterest').val('').trigger('chosen:updated');
