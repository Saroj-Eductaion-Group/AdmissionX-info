               ...
               <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach($coursetype as $item)
                            <tr>
                                <td><a href="{{ url('employee/coursetype', $item->id) }}">{{ $item->id }}</a></td>
                                <td><a href="{{ url('employee/coursetype', $item->id) }}">{{ $item->name }}</a></td>
                                <td>
                                    @if($item->eUserId)
                                ...
                                @if($storeEditUpdateAction == '1')
                                <td>
                                    <a href="{{ url('employee/coursetype/' . $item->id . '/edit') }}">
                                        <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                    </a><!--  /
                                    {!! Form::open([
                                        'method'=>'DELETE',
                                        'url' => ['employee/coursetype', $item->id],
