            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="slideDown" style="visibility:hidden">
                        ...
                        {!! Form::open(['url' => 'search/user-employee', 'class' => 'form-horizontal search-form', 'data-parsley-validate'=>'data-parsley-validate']) !!}
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <div class="hr-line-dashed"></div>
                                    <div class="col-md-12 text-right">      
                                        <a href="{{ URL::to('employee/users') }}" class="btn btn-default btn-sm">Close</a>
                    ...
                    <div class=""> <!-- table-responsive -->
                        ...
                        <table class="table table-bordered" >
                            ...
                            @foreach($users as $item)
                                <tr class="gradeX">
                                    <td><a href="{{ url('employee/users', $item->id) }}">{{ $item->id }}</a></td>
                                        <td><a href="{{ url('employee/users', $item->id) }}">{{ $item->firstname }}</a></td>
                                        <td>
                                            @if($item->lastname)
                                                {{ $item->lastname }}
                                            @else
                                        ...
                                        @if($storeEditUpdateAction == '1')
                                        <td>
                                           <a href="{{ url('employee/users/' . $item->id . '/edit') }}">
                                                <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                            </a>  <!-- /
                                            {!! Form::open([
                                                'method'=>'DELETE',
                                                'url' => ['employee/users', $item->id],
        ...
        $(document).ready(function() {
                        ...
                        if( data == 'no' ){
                            ...
                            $.each(data.userDataObj, function (key, item) {
                                ...
                                if( data.userDataObj[key].eUserId == null ){
                                    ...
                                    lastUpdatedBy = "<a class='' href='{!! URL::to('employee/users/"+ data.userDataObj[key].eUserId +"/') !!}' >"+ data.userDataObj[key].employeeFirstname+' '+data.userDataObj[key].employeeMiddlename+' '+data.userDataObj[key].employeeLastname+' (User Id:- '+data.userDataObj[key].eUserId+') <hr> Date & Time :- '+data.userDataObj[key].updated_at +"</a>";
                                }

                                $("tbody").append("<tr><td><a href='{!! URL::to('employee/users/"+ data.userDataObj[key].id +"/') !!}' >"+ data.userDataObj[key].id +"</a></td><td><a href='{!! URL::to('employee/users/"+ data.userDataObj[key].id +"/') !!}' >"+ data.userDataObj[key].firstname +"</a></td><td>"+ data.userDataObj[key].lastname +"</td><td>"+ data.userDataObj[key].phone +"</td> <td>"+ data.userDataObj[key].email +"</td><td>"+ data.userDataObj[key].userRoleName +"</td><td>"+ userStatusDataText +"</td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('employee/users/"+ data.userDataObj[key].id +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td></tr>");
                                    //<td><a href='{!! URL::to('employee/users/"+ data.userDataObj[key].id +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td>
            ...
            $('.resetfilter').on('click',function(e){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: "{{ URL::to('search/all-users-employee') }}",
                    success  : function(data) {
                        $(".thead").empty();
                        $(".tbody").empty();
                       $(".thead").append("<tr><td class='searchFilter'>User Id</td><td class='searchFilter'>First Name</td><td class='searchFilter'>Last Name</td><td class='searchFilter'>Phone</td><td class='searchFilter'>Email</td><td class='searchFilter'>User Type </td><td class='searchFilter'>User Status</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Action</td></tr>");
                       //<td class='searchFilter'>Actions</td>
                        ...
                        if( data.length == '' ){
                            ...
                            $.each(data, function (key, item) {
                               ...
                               $("tbody").append("<tr><td><a href='{!! URL::to('employee/users/"+ data[key].id +"/') !!}' >"+ data[key].id +"</a></td><td><a href='{!! URL::to('employee/users/"+ data[key].id +"/') !!}' >"+ data[key].firstname +"</a></td><td>"+ data[key].lastname +"</td><td>"+ data[key].phone +"</td> <td>"+ data[key].email +"</td><td>"+ data[key].userRoleName +"</td><td>"+ userStatusDataText +"</td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('employee/users/"+ data[key].id +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td></tr>");
                               //<td><a href='{!! URL::to('employee/users/"+ data[key].id +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td>
                            });
    ...
    <script type="text/javascript">
        $(document).on('click','.pagination > li > .currentCounter',function(){
            ...
            $.ajax({
                    ...
                    type     : "POST",
                    cache    : false,
                    url      : "{{ URL::to('search/user-employee') }}",
