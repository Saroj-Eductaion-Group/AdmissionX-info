        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div class="row">
                        <div class="col-md-6">
                            <h2>Chat Between College, Student &amp; Guest</h2>
                        </div>
                    ...
                    <div class="slideDown" style="visibility:hidden">
                        ...
                        {!! Form::open(['url' => 'search/employee-query-admissionx', 'class' => 'form-horizontal search-form', 'data-parsley-validate'=>'data-parsley-validate']) !!}
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <div class="col-md-12 text-right">      
                                        <a href="{{ URL::to('employee/query-bya') }}" class="btn btn-default btn-sm">Close</a>
                    ...
                    <div class=""> <!-- table-responsive -->
                        ...
                        <table class="table table-bordered" >
                            ...
                            @foreach($query as $item)
                                <tr class="gradeX">
                                    <td><a href="{{ url('employee/query-bya-detail', [$item->id, $item->queryflowtype]) }}">{{ $item->id }}</a></td>
                                    <td>
                                        @if( $item->subject )
                                           <a href="{{ url('employee/query-bya-detail', [$item->id, $item->queryflowtype]) }}">{{ $item->subject }}
                                                @if($item->querytypeinfo == 'pending')
                                    ...
                                    <td>
                                        {{ str_limit($item->message, 80) }}
                                    </td>
                                    <td>
                                        @if($item->queryflowtype == 'guest-to-admin')
                                            {{ $item->guestname }}
                                        @elseif( $item->queryflowtype == 'college-to-admin')
                                            {{ $item->U2FirstName }} 
                                        @elseif($item->queryflowtype == 'admin-to-college')
                                            {{ $item->U1FirstName }} {{ $item->U1LastName }}
                                        @elseif($item->queryflowtype == 'student-to-admin')
                                            {{ $item->U3FirstName }} {{ $item->U3MiddleName }}  {{ $item->U3LastName }}
                                        @elseif( $item->queryflowtype == 'admin-to-student' )
                                            {{ $item->U1FirstName }} {{ $item->U1LastName }} 
                                        @else
                                            --
                                        @endif
                                    ...
                                    <td>
                                        @if( str_slug($item->queryflowtype, ' ') != 'guest to admin' )
                                        <a href="{{ url('employee/query-bya-detail', [$item->id, $item->queryflowtype]) }}">
                                            <button type="submit" class="btn btn-primary btn-xs">Reply</button>
                                        </a> /
                                        @endif
                                        {!! Form::open([
                                            'method'=>'DELETE',
                                            'url' => ['employee/query', $item->id],
        ...
        $(document).ready(function() {
            ...
            $('.search-form').on('submit',function(e){
                ...
                $.ajax({
                    ...
                    success  : function(data) {
                        ...
                        $(".thead").empty();
                        $(".tbody").empty();
                        $(".thead").append("<tr><td class='searchFilter'>Id</td><td class='searchFilter'>Subject</td><td class='searchFilter'>Message</td><td class='searchFilter'>User Name</td><td class='searchFilter'>Chat Direction</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Actions</td></tr>");
                        ...
                        if( data == 'no' ){
                            ...
                            $.each(data.querySearchDataObj, function (key, item) {
                                ...
                                if( data.querySearchDataObj[key].message == ''){
                                     messageDataText = '--';
                                }else{
                                    messageDataText = data.querySearchDataObj[key].message;
                                }


                                if( data.querySearchDataObj[key].queryflowtype == 'guest-to-admin'){
                                     userNameDataText = data.querySearchDataObj[key].guestname;
                                }else if( data.querySearchDataObj[key].queryflowtype == 'college-to-admin'){
                                    userNameDataText = data.querySearchDataObj[key].U2FirstName;
                                }else if( data.querySearchDataObj[key].queryflowtype == 'admin-to-college'){
                                    userNameDataText = data.querySearchDataObj[key].U1FirstName+" "+data.querySearchDataObj[key].U1MiddleName+ " "+data.querySearchDataObj[key].U1LastName;
                                }else if( data.querySearchDataObj[key].queryflowtype == 'student-to-admin'){
                                    userNameDataText = data.querySearchDataObj[key].U3FirstName+" "+data.querySearchDataObj[key].U3MiddleName+" "+data.querySearchDataObj[key].U3LastName;
                                }else if( data.querySearchDataObj[key].queryflowtype == 'admin-to-student'){
                                    userNameDataText = data.querySearchDataObj[key].U1FirstName+" "+data.querySearchDataObj[key].U1MiddleName+ " "+data.querySearchDataObj[key].U1LastName;
                                }else{
                                ...
                                if( data.querySearchDataObj[key].queryflowtype == 'guest-to-admin' ){
                                    conditionalBody = "<a href='{!! URL::to('employee/query/delete/"+ data.querySearchDataObj[key].queryID +"') !!}' class='btn btn-danger btn-xs'>Delete</a>";
                                }else{
                                    conditionalBody = "<a href='{!! URL::to('employee/query-bya-detail/"+ data.querySearchDataObj[key].queryID+"/"+ data.querySearchDataObj[key].queryflowtype +"/') !!}' class='btn btn-primary btn-xs'>Reply</a>/<a href='{!! URL::to('employee/query/delete/"+ data.querySearchDataObj[key].queryID +"') !!}' class='btn btn-danger btn-xs'>Delete</a>";
                                }

                                if( data.querySearchDataObj[key].eUserId == null ){
                                    lastUpdatedBy = '<strong class="label label-warning">Not Updated</strong>';
                                }else{
                                    lastUpdatedBy = "<a class='' href='{!! URL::to('employee/users/"+ data.querySearchDataObj[key].eUserId +"/') !!}' >"+ data.querySearchDataObj[key].employeeFirstname+' '+data.querySearchDataObj[key].employeeMiddlename+' '+data.querySearchDataObj[key].employeeLastname+' (User Id:- '+data.querySearchDataObj[key].eUserId+') <hr> Date & Time :- '+data.querySearchDataObj[key].updated_at +"</a>";
                                }

                                $("tbody").append("<tr><td><a href='{!! URL::to('employee/query/"+ data.querySearchDataObj[key].queryID +"/') !!}' >"+ data.querySearchDataObj[key].queryID +"</a></td><td><a href='{!! URL::to('employee/query-bya-detail/"+ data.querySearchDataObj[key].queryID+"/"+ data.querySearchDataObj[key].queryflowtype +"/') !!}' >"+ subjectDataText +"</a></td><td>"+ messageDataText +"</td><td>"+ userNameDataText +"</td><td>"+ chatDirectionText +"</td><td>"+ lastUpdatedBy +" </td><td>"+conditionalBody+"</td></td></tr>");
            ...
            $('.resetfilter').on('click',function(e){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: "{{ URL::to('search/employee-all-query-admissionx') }}",
                    success  : function(data) {
                        $(".thead").empty();
                        $(".tbody").empty();
                       $(".thead").append("<tr><td class='searchFilter'>Id</td><td class='searchFilter'>Subject</td><td class='searchFilter'>Message</td><td class='searchFilter'>User Name</td><td class='searchFilter'>Chat Direction</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Actions</td></tr>");
                            ...
                            $.each(data, function (key, item) {
                               ...
                               if( data[key].subject == ''){
                                ...
                                if( data[key].message == ''){
                                    ...
                                    messageDataText = data[key].message;
                                }


                                if( data[key].queryflowtype == 'guest-to-admin'){
                                     userNameDataText = data[key].guestname;
                                }else if( data[key].queryflowtype == 'college-to-admin'){
                                    userNameDataText = data[key].U2FirstName;
                                }else if( data[key].queryflowtype == 'admin-to-college'){
                                    userNameDataText = data[key].U1FirstName+" "+data[key].U1MiddleName+ " "+data[key].U1LastName;
                                }else if( data[key].queryflowtype == 'student-to-admin'){
                                    userNameDataText = data[key].U3FirstName+" "+data[key].U3MiddleName+" "+data[key].U3LastName;
                                }else if( data[key].queryflowtype == 'admin-to-student'){
                                    userNameDataText = data[key].U1FirstName+" "+data[key].U1MiddleName+ " "+data[key].U1LastName;
                                }else{
                                    userNameDataText = '<strong class="label label-warning">Not Updated</strong>';
                                }
                                ...
                                if( data[key].queryflowtype == 'guest-to-admin' ){
                                    conditionalBody = "<a href='{!! URL::to('employee/query/delete/"+ data[key].queryID +"') !!}' class='btn btn-danger btn-xs'>Delete</a>";
                                }else{
                                    conditionalBody = "<a href='{!! URL::to('employee/query-bya-detail/"+ data[key].queryID+"/"+ data[key].queryflowtype +"/') !!}' class='btn btn-primary btn-xs'>Reply</a>/<a href='{!! URL::to('employee/query/delete/"+ data[key].queryID +"') !!}' class='btn btn-danger btn-xs'>Delete</a>";
                                }
                                ...
                                $("tbody").append("<tr><td><a href='{!! URL::to('employee/query/"+ data[key].queryID +"/') !!}' >"+ data[key].queryID +"</a></td><td><a href='{!! URL::to('employee/query-bya-detail/"+ data[key].queryID+"/"+ data[key].queryflowtype +"/') !!}' >"+ subjectDataText +"</a></td><td>"+ messageDataText +"</td><td>"+ userNameDataText +"</td><td>"+ chatDirectionText +"</td><td>"+ lastUpdatedBy +" </td><td>"+conditionalBody+"</td></td></tr>");
    ...
    <script type="text/javascript">
        $(document).on('click','.pagination > li > .currentCounter',function(){
            ...
            $.ajax({
                    ...
                    type     : "POST",
                    cache    : false,
                    url      : "{{ URL::to('search/employee-query-admissionx') }}",
