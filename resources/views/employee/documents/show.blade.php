...
<!-- <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-12">
        <h2>Document Details <a href="{{ url('employee/documents/create') }}" class="btn btn-primary pull-right btn-sm">Add New Document</a></h2>
    </div>
</div>
 -->
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
               <!-- <a href="{{ url('employee/documents') }}" class="btn btn-warning pull-right btn-sm"><i class="fa fa-arrow-left"></i> Back</a> -->
               <table class="table table-bordered">
                    ...
                    @foreach( $document as $document)
                        ...
                        <tr>
                            ...
                            <td>
                                @if($document->eUserId)
                                <a href="{{ url('employee/users', $document->eUserId) }}">{{ $document->employeeFirstname }} {{ $document->employeeMiddlename}} {{ $document->employeeLastname}} (ID:- {{ $document->eUserId}}) Date & Time:-  {{ $document->updated_at}}</a>
                                @else
                        ...
                        <tr>
                            ...
                            <td>@if( $document->userID)
                                    <a href="{{ url('employee/users') }}/{{ $document->userID }}" title="{{ $document->firstname }} {{ $document->lastname }}">{{ $document->firstname }} {{ $document->lastname }} </a>
                                @else
                                    <span class="label label-warning">Not Updated Yet</span>
                                @endif
                            </td>
