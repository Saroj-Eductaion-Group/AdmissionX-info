...
<div class="row wrapper border-bottom white-bg page-heading">
    <!--  <div class="col-lg-12">
        <h2>Application Status Message Details <a href="{{ url('employee/applicationstatusmessage/create') }}" class="btn btn-primary pull-right btn-sm">Add New Application Status Message</a></h2>
    </div> -->
</div>
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="slideDown" style="visibility:hidden">
                        ...
                        {!! Form::open(['url' => 'search/employee-application-remarks', 'class' => 'form-horizontal search-form', 'data-parsley-validate'=>'data-parsley-validate']) !!}
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <div class="hr-line-dashed"></div>
                                    <div class="col-md-12 text-right">      
                                        <a href="{{ URL::to('employee/applicationstatusmessage') }}" class="btn btn-default btn-sm">Close</a>
                    ...
                    <div class=""> <!-- table-responsive -->
                        ...
                        <table class="table table-bordered" >
                            ...
                            @foreach($applicationstatusmessage as $item)
                                <tr class="gradeX">
                                    <td>
                                        <a href="{{ url('employee/applicationstatusmessage', $item->id) }}">{{ $item->id }}</a>
                                    </td>
                                    <td>
                                        <a href="{{ url('employee/application', $item->applicationId) }}">{{ $item->applicationID }}</a>
                                    </td>
                                    <td>
                                        <a href="{{ url('employee/studentprofile', $item->studentprofileId) }}">{{ $item->studentUserFirstName }} {{ $item->studentUserMiddleName }} {{ $item->studentUserLastName }}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{ url('employee/applicationstatusmessage', $item->id) }}">
                                            <button type="submit" class="btn btn-info btn-xs">Show</button>
                                        </a>
                                    </td>
                                   <!--  @if($storeEditUpdateAction == '1')
                                   <td>
                                        <a href="{{ url('employee/applicationstatusmessage/' . $item->id . '/edit') }}">
                                            <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                        </a> /
                                        {!! Form::open([
                                            'method'=>'DELETE',
                                            'url' => ['employee/applicationstatusmessage', $item->id],
        ...
        $(document).ready(function() {
            ...
            $('.resetfilter').on('click',function(e){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: "{{ URL::to('search/employee-all-application-remarks') }}",
                    success  : function(data) {
                        $(".thead").empty();
                        $(".tbody").empty();
                       $(".thead").append("<tr><td class='searchFilter'>S.No</td><td class='searchFilter'>Application Id</td><td class='searchFilter'>Student Name</td><td class='searchFilter'>College Name</td><td class='searchFilter'>Application Status</td><td class='searchFilter'>Remark Application</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Action</td></tr>");
                        ...
                        if( data.length == '' ){
                            ...
                            $.each(data, function (key, item) {
                               ...
                               if(data[key].applicationId == null ){
                                ...
                                $("tbody").append("<tr><td><a href='{!! URL::to('employee/applicationstatusmessage/"+ data[key].id +"/') !!}' >"+ data[key].id +"</a></td><td><a href='{!! URL::to('employee/application/"+ data[key].applicationId +"/') !!}' >"+ applicationIdDataText +"</a></td><td><a href='{!! URL::to('employee/studentprofile/"+ data[key].studentprofileId +"/') !!}' >"+ studentnameDataText +"</a></td><td><a href='{!! URL::to('employee/collegeprofile/"+ data[key].collegeprofileID +"/') !!}' >"+ collegeNameDataText +"</a></td><td>"+ applicationStatusText +"</td> <td>"+ remarksApplicationDataText +"</td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('employee/applicationstatusmessage/"+ data[key].id +"/') !!}' class='btn btn-primary btn-xs'>Show</a></td></tr>");
                            });
    ...
    <script type="text/javascript">
        $(document).on('click','.pagination > li > .currentCounter',function(){
            ...
            $.ajax({
                    ...
                    type     : "POST",
                    cache    : false,
                    url      : "{{ URL::to('search/employee-application-remarks') }}",
