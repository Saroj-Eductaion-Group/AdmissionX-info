            ...
            <div class="ibox-content">
               <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><a href="{{ url('employee/userrole', $item->id) }}">{{ $item->id }}</a></td>
                            <td><a href="{{ url('employee/userrole', $item->id) }}">{{ $item->name }}</a></td>
                            <td>
                                @if($item->eUserId)
                            ...
                            @if( $storeEditUpdateAction == '1' )
                            <td>
                                <a href="{{ url('employee/userrole/' . $item->id . '/edit') }}">
                                    <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                </a><!--  /
                                {!! Form::open([
                                    'method'=>'DELETE',
                                    'url' => ['employee/userrole', $item->id],
