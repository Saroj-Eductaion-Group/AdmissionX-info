    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
               <!-- <a href="{{ url('administrator/galleries') }}" class="btn btn-warning pull-right btn-sm"><i class="fa fa-arrow-left"></i> Back</a> -->
               <table class="table table-bordered">
                    ...
                    @foreach( $gallery as $gallery)
                        ...
                        <tr>
                            ...
                            <td>
                                @if($gallery->eUserId)
                                <a href="{{ url('administrator/users', $gallery->eUserId) }}">{{ $gallery->employeeFirstname }} {{ $gallery->employeeMiddlename}} {{ $gallery->employeeLastname}} (ID:- {{ $gallery->eUserId}}) Date & Time:-  {{ $gallery->updated_at}}</a>
                                @else
                         ...
                         <tr>
                            ...
                            <td>@if( $gallery->userID)
                                    <a href="{{ url('administrator/users') }}/{{ $gallery->userID }}" title="{{ $gallery->firstname }} {{ $gallery->lastname }}">{{ $gallery->firstname }} {{ $gallery->lastname }} </a>
                                @else
                                    <span class="label label-warning">Not Updated Yet</span>
                                @endif
                            </td>
