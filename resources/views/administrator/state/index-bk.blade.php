...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-12">
        <h2>State Details
            <a href="{{ url('administrator/state/create') }}" class="btn btn-primary pull-right btn-sm">Add New State</a>
        </h2>
    </div>
</div>
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="slideDown" style="visibility:hidden">
                        ...
                        {!! Form::open(['url' => 'search/state-name', 'class' => 'form-horizontal search-form', 'data-parsley-validate'=>'data-parsley-validate']) !!}
                                ...
                                <div class="panel-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" class="form-control stateName" name="stateName" placeholder="Enter state name here" data-parsley-error-message="Please enter state name" data-parsley-trigger="change">
                                        </div> 
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4 text-right">      
                                            <a href="{{ URL::to('administrator/state') }}" class="btn btn-default btn-sm">Close</a>
                    ...
                    <div class=""> <!-- table-responsive -->
                        ...
                        @if( $state == '0' )
                            <input type="text" class="result-zero hide" value="{{ $state }}">
                        ...
                        <table class="table table-bordered" >
                            ...
                            @foreach($state as $item)
                                <tr class="gradeX">
                                    <td><a href="{{ url('administrator/state', $item->id) }}">{{ $item->id }}</a></td>
                                    <td><a href="{{ url('administrator/state', $item->id) }}">{{ $item->name }}</a></td>
                                    ...
                                    <td>
                                        <a href="{{ url('administrator/state/' . $item->id . '/edit') }}">
                                            <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                        </a> <!-- /
                                        {!! Form::open([
                                            'method'=>'DELETE',
                                            'url' => ['administrator/state', $item->id],
                        ...
                        <div class="row indexPagination">
                            <div class="col-md-12">
                                <div class="pull-right custom-pagination">{!! $state->render() !!}</div>
                            </div>
                        </div>
                        @endif
        ...
        $(document).ready(function() {
            ...
            $('.search-form').on('submit',function(e){
                ...
                $.ajax({
                    ...
                    success  : function(data) {
                        ...
                        $(".thead").empty();
                        $(".tbody").empty();
                        $(".thead").append("<tr><td class='searchFilter'>Id</td><td class='searchFilter'>State Name</td><td class='searchFilter'>Country Name</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Actions</td></tr>");
                        ...
                        if( data == 'no' ){
                            ...
                            $.each(data.stateSearchDataObj, function (key, item) {

                                if( data.stateSearchDataObj[key].countryName == ''){
                                     countryDataText = '<strong class="label label-warning">Not Updated</strong>';
                                }else{
                                    countryDataText = data.stateSearchDataObj[key].countryName;
                                }

                                if( data.stateSearchDataObj[key].name == ''){
                                     stateNameDataText = '<strong class="label label-warning">Not Updated</strong>';
                                }else{
                                    stateNameDataText = data.stateSearchDataObj[key].name;
                                }


                                if( data.stateSearchDataObj[key].eUserId == null ){
                                    lastUpdatedBy = '<strong class="label label-warning">Not Updated</strong>';
                                }else{
                                    lastUpdatedBy = "<a class='' href='{!! URL::to('administrator/users/"+ data.stateSearchDataObj[key].eUserId +"/') !!}' >"+ data.stateSearchDataObj[key].employeeFirstname+' '+data.stateSearchDataObj[key].employeeMiddlename+' '+data.stateSearchDataObj[key].employeeLastname+' (User Id:- '+data.stateSearchDataObj[key].eUserId+') <hr> Date & Time :- '+data.stateSearchDataObj[key].updated_at +"</a>";
                                }
                               
                                
                                $("tbody").append("<tr><td><a href='{!! URL::to('administrator/state/"+ data.stateSearchDataObj[key].stateId +"/') !!}' >"+ data.stateSearchDataObj[key].stateId +"</a></td><td><a href='{!! URL::to('administrator/state/"+ data.stateSearchDataObj[key].stateId +"/') !!}' >"+ stateNameDataText +"</a></td><td>"+ countryDataText +"</td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('administrator/state/"+ data.stateSearchDataObj[key].stateId +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td></tr>");
                                
                            });

                            //Create html pagination desgin
                            if( data.stateSearchDataObj1 > 1 ){
                                var HTML = '';
                                HTML +='<ul class="pagination">';
                                HTML +='<li class="active"><span class="currentCounter">1</span></li>';
                                if( data.stateSearchDataObj1 < 8 ){
                                    for(var i=2; i <= data.stateSearchDataObj1; i++){
                                ...
                                if( data.stateSearchDataObj1 < 8 ){
                                    ...
                                    var lessTwo = data.stateSearchDataObj1-1;
                                    for(var i=lessTwo; i <= data.stateSearchDataObj1; i++){
                                        HTML +='<li><span class="currentCounter">'+ i +'</span></li>';
                                    } 
                                }
            ...
            $('.resetfilter').on('click',function(e){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: "{{ URL::to('search/all-state-name') }}",
                    success  : function(data) {
                        $(".thead").empty();
                        $(".tbody").empty();
                       $(".thead").append("<tr><td class='searchFilter'>Id</td><td class='searchFilter'>State Name</td><td class='searchFilter'>Country Name</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Actions</td></tr>");
                            ...
                            $.each(data, function (key, item) {
                               ...
                               if( data[key].countryName == ''){
                                ...
                                if( data[key].name == ''){
                                    ...
                                    stateNameDataText = data[key].name;
                                }
                                ...
                                $("tbody").append("<tr><td><a href='{!! URL::to('administrator/state/"+ data[key].stateId +"/') !!}' >"+ data[key].stateId +"</a></td><td><a href='{!! URL::to('administrator/state/"+ data[key].stateId +"/') !!}' >"+ stateNameDataText +"</a></td><td>"+ countryDataText +"</td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('administrator/state/"+ data[key].stateId +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a></td></tr>");
    ...
    <script type="text/javascript">
        $(document).on('click','.pagination > li > .currentCounter',function(){
            ...
            $.ajax({
                    ...
                    type     : "POST",
                    cache    : false,
                    url      : "{{ URL::to('search/state-name') }}",
                    data     : { stateName: stateName,  countryName:countryName, currentNode: currentNode },
                    dataType : "json",
                    success: function(data) {
                        $('.prevFilter').show();
                        $('.spiner-example').addClass('hide');
                        ...
                        if( data == 'no' ){
                            ...
                            if( data.currentNode >= 8 ){
                                ...
                                for( var adds = plusOne; adds <= addInCurrentNode; adds++ ){
                                    if( data.stateSearchDataObj1 > adds ){
