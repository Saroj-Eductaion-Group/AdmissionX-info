            				...
            				<table class="table table-bordered">
            					...
            					<tbody>
            						@foreach( $getAllQueriesBetween as $item)
            							<tr>
            								<td><a href="{{ URL('/administrator/query-college-student-details', [$item->chatkey, $item->id]) }}">{{ $item->id }}</a></td>
            								<td><a href="{{ URL('/administrator/query-college-student-details', [$item->chatkey, $item->id]) }}">{{ $item->subject }}</a></td>
            								<td>{{ str_limit($item->message, 50) }}</td>
                                            <td>
                                                @if( $item->collegeprofileID )
                                                  <a href="{{ url('administrator/collegeprofile', $item->collegeprofileID) }}">{{ $item->U2FirstName }}</a>
                                                @else
            								...
            								<td><a href="{{ URL('/administrator/query-college-student-details', [$item->chatkey, $item->id]) }}" class="btn btn-xs btn-info">Show</a></td>
            							</tr>
            						@endforeach
            					</tbody>
            				</table>
