...
@endsection
@section('content')
{{--*/ $validateUserRoleCall = $fetchDataServiceController->validateUserRoleCall('Country'); /*--}}
        ...
        @if(Auth::check())
            @if(Auth::user()->userrole_id == 4)
                @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->create == '1'))
                    <a href="{{ url($fetchDataServiceController->routeCall().'/country/create') }}" class="btn btn-primary pull-right btn-sm">Add New Country</a>
                @endif
            @else
                <a href="{{ url($fetchDataServiceController->routeCall().'/country/create') }}" class="btn btn-primary pull-right btn-sm">Add New Country</a>
            @endif
    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-content">
                ...
                <form action="{{ URL::to($fetchDataServiceController->routeCall().'/country') }}" method="GET">
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control countryName" name="countryName" placeholder="Enter Country name here" data-parsley-error-message="Please enter Country name" data-parsley-trigger="change" value="{{Request::get('countryName') }}">
                        </div>  
                        ...
                        <div class="col-md-3 pull-right text-right margin-top20">
                            <a href="{{ URL::to($fetchDataServiceController->routeCall().'/country') }}" class="btn btn-md btn-primary">Clear</a>
...
<div class="row wrapper border-bottom page-heading margin-top20">
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <label class="pull-right">Total Result :- {{ $country->total() }}</label>
                <table class="table table-hover">
                    ...
                    <tbody class="tbody">
                        ...
                        <tr>
                            <td><a href="{{ url($fetchDataServiceController->routeCall().'/country', $item->id) }}">{{ $item->id }}</a></td>
                            <td><a href="{{ url($fetchDataServiceController->routeCall().'/country', $item->id) }}">{{ $item->name }}</a>
                            	@include('common-partials.common-fileds-index-partial')
                            </td>
                            <td>
                                @if($item->logoimage != '')
                                    <img class="img-thumbnail img-responsive" src="{{ asset('common-logo') }}/{{ $item->logoimage }}" style="width: 120px; height: 120px; ">
                                @else
                            ...
                            @if(Auth::check())
                                @if(Auth::user()->userrole_id == 4)
                                    ...
                                    @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->show == '1'))
                                        <a href="{{ url($fetchDataServiceController->routeCall().'/country/' . $item->id) }}">
                                            <button type="submit" class="btn btn-info btn-xs">Show</button>
                                        </a>
                                    @endif
                                    @if((isset($validateUserRoleCall)) && (sizeof($validateUserRoleCall) > 0) && ($validateUserRoleCall[0]->edit == '1'))
                                        <a href="{{ url($fetchDataServiceController->routeCall().'/country/' . $item->id . '/edit') }}">
        		...
        		<div class="pagination">{!! $country->appends(\Input::except('page'))->render() !!}</div>
            </div>
        </div>
    </div>
</div>
