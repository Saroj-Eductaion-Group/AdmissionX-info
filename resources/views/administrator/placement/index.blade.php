            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="slideDown" style="visibility:hidden">
                        ...
                        {!! Form::open(['url' => 'search/placement', 'class' => 'form-horizontal search-form', 'data-parsley-validate'=>'data-parsley-validate']) !!}
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <div class="hr-line-dashed"></div>
                                    <div class="col-md-12 text-right">      
                                        <a href="{{ URL::to('administrator/placement') }}" class="btn btn-default btn-sm">Close</a>
                    ...
                    <div class=""> <!-- table-responsive -->
                        ...
                        <table class="table table-bordered" >
                            ...
                            @foreach($placement as $item)
                                <tr class="gradeX">
                                    <td><a href="{{ url('administrator/placement', $item->id) }}">{{ $item->id }}</a></td>
                                    <td>
                                        @if($item->numberofrecruitingcompany)
                                            <a href="{{ url('administrator/placement', $item->id) }}">{{ $item->numberofrecruitingcompany }}</a>
                                        @else
                                    ...
                                    <td>
                                     ...
                                     <td>
                                        @if( !empty($item->collegeprofileID) )
                                            <a href="{{ url('administrator/collegeprofile', $item->collegeprofileID) }}">{{ $item->collegeUserFirstName }} ( {{ $item->collegeUserEstYear }} )</a>
                                        @else
                                            <span class="label label-warning">Not Updated Yet</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ url('administrator/placement/' . $item->id . '/edit') }}">
                                            <button type="submit" class="btn btn-primary btn-xs">Update</button>
                                        </a> /
                                        {!! Form::open([
                                            'method'=>'DELETE',
                                            'url' => ['administrator/placement', $item->id],
        ...
        $(document).ready(function() {
            ...
            $('.resetfilter').on('click',function(e){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: "{{ URL::to('search/all-placement') }}",
                    success  : function(data) {
                        $(".thead").empty();
                        $(".tbody").empty();
                       $(".thead").append("<tr><td class='searchFilter'>Id</td><td class='searchFilter'>Number Of Recruiting Company</td><td class='searchFilter'>Number Of Placement Last Year</td><td class='searchFilter'>CTC Highest</td><td class='searchFilter'>CTC Lowest</td><td class='searchFilter'>CTC Average</td><td class='searchFilter'>College Profile</td><td class='searchFilter'>Last Updated By</td><td class='searchFilter'>Actions</td></tr>");
                        ...
                        if( data.length == '' ){
                            ...
                            $.each(data, function (key, item) {
                               ...
                               if( data[key].numberofplacementlastyear == ''){
                                ...
                                $("tbody").append("<tr><td><a href='{!! URL::to('administrator/placement/"+ data[key].placementId +"/') !!}' >"+ data[key].placementId +"</a></td><td><a href='{!! URL::to('administrator/placement/"+ data[key].placementId +"/') !!}' >"+ numberofrecruitingcompanyData +"</a></td><td>"+ numberofplacementlastyearData +"</td> <td>"+ ctcHighestDataText +"</td> <td>"+ ctcLowestDataText +"</td> <td>"+ ctcAverageDataText +"</td><td><a href='{!! URL::to('administrator/collegeprofile/"+ data[key].collegeprofileID +"/') !!}' >"+ data[key].usersFirstName +"</a></td><td>"+ lastUpdatedBy +" </td><td><a href='{!! URL::to('administrator/placement/"+ data[key].placementId +"/edit/') !!}' class='btn btn-primary btn-xs'>update</a>/<a href='{!! URL::to('administrator/placement/delete/"+ data[key].placementId +"') !!}' class='btn btn-danger btn-xs'>Delete</a></td></tr>");
                            });
