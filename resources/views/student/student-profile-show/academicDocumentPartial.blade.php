							...
							<div class="grid-boxes-caption">
								@if( $item['documentsName'] )
                                    @if( $item['documentsName'] != 'no-image-upload' )
                                        @if( $item['ext'] != 'pdf' )
                                            <a href="{{asset('document/')}}/{{ $slugUrl }}/{{ $item['documentsName'] }}" rel="gallery" class="fancybox img-hover-v1" title="{{ $item['description'] }}">
                                                <span><img class="img-responsive" src="{{asset('document/')}}/{{ $slugUrl }}/{{ $item['documentsName'] }}" alt="{{ $item['documentsName'] }}"></span>
                                            </a>
                                        @else
		...
		<div class="row">
			<div class="col-md-12">
				<div class="headline text-center">
					<h4>Document's Not Updated Yet.</h4>
				</div>
			</div>
		</div>
		@endif
